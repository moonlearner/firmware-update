...
class QuantaSkylake(object):
    ...
    def validate(host, username, password):
        ...
        print('Start  ' + temp)
        redfishapi = 'https://[' + host + ']/redfish/v1/'
        redfishheader = {
            'Content-Type': 'application/json',
            'User-Agent': 'curl/7.54.0',
            'Host': '[' + host.split('%')[0] + ']'
        ...
        # Attempt to connect
        try:
            session = requests.get(redfishapi + 'Systems/Self', auth=(username, password), verify=False,
    ...
    def getSystemsJSON(self):
        ...
        session = requests.get(self.redfishapi + 'Systems/Self', auth=(self.username, self.password), verify=False, headers=self.redfishheader)
        try:
    ...
    def getManagersJSON(self):
        session = requests.get(self.redfishapi + 'Managers/Self', auth=(self.username, self.password), verify=False, headers=self.redfishheader)
        # Decode JSON to Dictionary
        j = session.json()
        # Store it
    ...
    def getRegistriesJSON(self):
        session = requests.get(self.redfishapi + 'Registries/Self', auth=(self.username, self.password), verify=False, headers=self.redfishheader)
        # Decode JSON to Dictionary
        j = session.json()
        return j
    ...
    def getBIOSJSON(self):
        session = requests.get(self.redfishapi + 'Systems/Self/Bios', auth=(self.username, self.password), verify=False, headers=self.redfishheader)
        try:
            j = session.json()
        except:
            return {'error':'error'}
    ...
    def getBIOSJSONRegistries(self):
        # Get BIOS Registries
        session = requests.get(self.redfishapi + 'Registries/BiosAttributeRegistry0.0.0.0.json', auth=(self.username, self.password), verify=False, headers=self.redfishheader)
        j = session.json()
        return j
    ...
    def getJSON(self, inputurl):
        if self.redfishapi in inputurl:
            url = str(inputurl)
        else:
            url = self.redfishapi + str(inputurl)
...
class Q72D(QuantaSkylake):
    ...
    def setTime(self):
        ...
        try:
            session = requests.patch(self.redfishapi + 'Managers/Self/LogServices/SEL', auth=(self.username, self.password),
                                   verify=False, headers=self.redfishheader,
                                   data=senddata)
        except:
            pass
